...
# Calcular poder_inf si umbral_inf no es nulo
if(!is.null(x1)) {
  poder_inf <- pnorm(x1, mean = media_poblacional, sd = SE, lower.tail = TRUE)
}

# Calcular poder_sup si umbral_sup no es nulo
if(!is.null(x2)) {
...
Podemos apreciar de la gráfica que el poder se acerca a 1 cuando los litros son distintos a la hipótesis nula que corresponden a 10 Litros.
...
Considerando que nos preguntan sobre la cantidad de bidones y que además nos entregan el poder estadístico y el nivel de significación α podemos utilizar la función pwr.norm.test() otorgándole valor NULL al tamaño de la muestra para que nos lo retorne y sepamos la cantidad de bidones.
...
#Tamaño del efecto d de Cohen (divide por 1, porque desviación estándar = 1) en donde restamos la media verdadera de la pregunta 2 con la media de nuestra hipotesis nula para luego dividirlo por la desviacion estándar
d = (media_verdadera - media_poblacional)/desviacion_estandar
...
H0 = $n_{1} = n_{2};$ $n_{1}:$ 100 , $n_{2}:$ n esperados para la muestra
